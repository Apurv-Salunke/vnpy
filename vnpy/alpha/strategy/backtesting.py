...
class BacktestingEngine:
    ...
    def set_parameters(
        ...
        for vt_symbol in vt_symbols:
            ...
            if not setting:
                logger.warning(f"Cannot find trading configuration for contract {vt_symbol}, please check!")
                continue
    ...
    def run_backtesting(self) -> None:
        ...
        for dt in dts:
            try:
                ...
                logger.info("Exception triggered, backtesting terminated")
    ...
    def calculate_result(self) -> pl.DataFrame | None:
        ...
        logger.info("Start calculating daily mark-to-market PnL")
        ...
        logger.info("Daily mark-to-market PnL calculation complete")
    ...
    def calculate_statistics(self) -> dict:
        ...
        if df is not None:
            ...
            if not positive_balance:
                logger.info("Liquidation occurred during backtesting (capital <= 0), cannot calculate strategy statistics")
        ...
        logger.info(f"Profitable trading days:  {profit_days}")
        logger.info(f"Losing trading days:  {loss_days}")

        logger.info(f"Starting capital:  {self.capital:,.2f}")
        logger.info(f"Ending balance:  {end_balance:,.2f}")

        logger.info(f"Total return rate:  {total_return:,.2f}%")
        logger.info(f"Annualized return:  {annual_return:,.2f}%")
        logger.info(f"Max drawdown:   {max_drawdown:,.2f}")
        logger.info(f"Percentage max drawdown: {max_ddpercent:,.2f}%")
        logger.info(f"Longest drawdown duration (days):   {max_drawdown_duration}")

        logger.info(f"Total PnL:  {total_net_pnl:,.2f}")
        logger.info(f"Total commission:  {total_commission:,.2f}")
        logger.info(f"Total turnover:  {total_turnover:,.2f}")
        logger.info(f"Total trade count:  {total_trade_count}")

        logger.info(f"Average daily PnL:  {daily_net_pnl:,.2f}")
        logger.info(f"Average daily commission:  {daily_commission:,.2f}")
        logger.info(f"Average daily turnover:  {daily_turnover:,.2f}")
        logger.info(f"Average daily trade count:  {daily_trade_count}")

        logger.info(f"Average daily return rate:  {daily_return:,.2f}%")
    ...
    def get_signal(self) -> pl.DataFrame:
        ...
        if not self.datetime:
            self.write_log("Data replay not started yet, cannot load model predictions")
            return pl.DataFrame()
